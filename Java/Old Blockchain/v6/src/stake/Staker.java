...
public class Staker {
    ...
    public static void forgeRandom() {
        ...
        Ancestors a = new Ancestors(Blockchain.getLayer(fromHex(Integer.toHexString(Integer.valueOf(getHex(height), 16) - 1))).getHashes());
        Payload p = new Payload(TxPool.getTxs());

        for(int i = 0; i < x; i++){
            Blockchain.addBlock(new Block(height, randomString(10), MINER_WIF, p, a), Integer.valueOf(getHex(height), 16));
        }
    }
    ...
    public static void forge(int blockCount) {
        ...
        for(int i = 0; i < blockCount; i++){
            Blockchain.addBlock(new Block(height, randomString(3), MINER_WIF, p, a), Integer.valueOf(getHex(height), 16));
        }
    }
