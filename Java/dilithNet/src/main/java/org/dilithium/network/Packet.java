...
public class Packet {
    ...
    public Packet(byte[] rlpEncoded) {
        this.rlpEncoded = rlpEncoded;
    }

    /* Construct new packet */
    public Packet(byte[] target, int messageType, byte[] payload, byte[] privkey) {
        this.target = target;
    ...
    public Packet(int messageType, byte[] payload, byte[] privkey) {
        this.target = null;
        this.messageType = messageType;
        this.payload = payload;
    ...
    public synchronized void rlpParse() {
        ...
        try {
            ...
            for (RLPElement rlpElement : packet) {
                if(!(rlpElement instanceof RLPItem)) throw new RuntimeException("Packet RLP elements shouldn't be lists");
            }
