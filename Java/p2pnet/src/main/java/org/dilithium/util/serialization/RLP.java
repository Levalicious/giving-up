...
public class RLP {
    ...
    private static void fullTraverse(byte[] msgData, int level, int startPos,
        ...
        try {
            ...
            while (pos < endPos) {
                ...
                // null item
                if ((msgData[pos] & 0xFF) == OFFSET_SHORT_ITEM) {
                    byte[] item = util.ByteUtil.EMPTY_BYTE_ARRAY;
                    RLPItem rlpItem = new RLPItem(item);
                    rlpList.add(rlpItem);
                    pos += 1;
                    continue;
    ...
    public static byte[] encode(Object input) {
        ...
        if (val.isList()) {
            ...
            byte[] output = util.ByteUtil.EMPTY_BYTE_ARRAY;
    ...
    private static byte[] toBytes(Object input) {
        if (input instanceof byte[]) {
            ...
            return (inputLong == 0) ? util.ByteUtil.EMPTY_BYTE_ARRAY : asUnsignedByteArray(BigInteger.valueOf(inputLong));
        } else if (input instanceof Integer) {
            Integer inputInt = (Integer) input;
            return (inputInt == 0) ? util.ByteUtil.EMPTY_BYTE_ARRAY : asUnsignedByteArray(BigInteger.valueOf(inputInt));
        } else if (input instanceof BigInteger) {
            BigInteger inputBigInt = (BigInteger) input;
            return (inputBigInt.equals(BigInteger.ZERO)) ? util.ByteUtil.EMPTY_BYTE_ARRAY : asUnsignedByteArray(inputBigInt);
