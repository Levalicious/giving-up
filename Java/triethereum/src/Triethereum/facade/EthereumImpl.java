 ...
 */
package Triethereum.facade;

import org.apache.commons.lang3.ArrayUtils;
import Triethereum.config.BlockchainConfig;
import Triethereum.config.CommonConfig;
import Triethereum.config.SystemProperties;
import Triethereum.core.*;
import Triethereum.core.PendingState;
import Triethereum.core.Repository;
import Triethereum.crypto.ECKey;
import Triethereum.listener.CompositeEthereumListener;
import Triethereum.listener.EthereumListener;
import Triethereum.listener.EthereumListenerAdapter;
import Triethereum.listener.GasPriceTracker;
import Triethereum.manager.AdminInfo;
import Triethereum.manager.BlockLoader;
import Triethereum.manager.WorldManager;
import Triethereum.mine.BlockMiner;
import Triethereum.net.client.PeerClient;
import Triethereum.net.rlpx.Node;
import Triethereum.net.server.ChannelManager;
import Triethereum.net.shh.Whisper;
import Triethereum.net.submit.TransactionExecutor;
import Triethereum.net.submit.TransactionTask;
import Triethereum.sync.SyncManager;
import Triethereum.util.ByteUtil;
import Triethereum.vm.program.ProgramResult;
import Triethereum.vm.program.invoke.ProgramInvokeFactory;
...
public class EthereumImpl implements Ethereum, SmartLifecycle {
    ...
    @Override
    public Triethereum.facade.Blockchain getBlockchain() {
        return (Triethereum.facade.Blockchain) worldManager.getBlockchain();
    }
    ...
    public BlockSummary replayBlock(Block block) {
        ...
        try {
            for (Transaction tx : block.getTransactionsList()) {
                ...
                Triethereum.core.TransactionExecutor executor = new Triethereum.core.TransactionExecutor(
    ...
    private Triethereum.core.TransactionExecutor callConstantImpl(Transaction tx, Block block) {
    ...
    @Override
    public Triethereum.facade.Repository getRepository() {
        return worldManager.getRepository();
    }

    @Override
    public Triethereum.facade.Repository getLastRepositorySnapshot() {
        return getSnapshotTo(getBlockchain().getBestBlock().getStateRoot());
    }

    @Override
    public Triethereum.facade.Repository getPendingState() {
        return worldManager.getPendingState().getRepository();
    }

    @Override
    public Triethereum.facade.Repository getSnapshotTo(byte[] root) {

        Repository repository = (Repository) worldManager.getRepository();
        Triethereum.facade.Repository snapshot = repository.getSnapshotTo(root);
