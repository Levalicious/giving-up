...
public class Transaction implements UTXOPool, TxBase, Serializable {
    ...
    public byte[] calcFee() {
        return fromHex(BIUtil.toBI(value).divide(new BigInteger("100")).toString(16));

    }

    public byte[] calcFeeWithData() {
        return fromHex(BIUtil.toBI(value).divide(new BigInteger("100")).multiply(new BigInteger(Long.toString(data.length))).toString(16));
    }
