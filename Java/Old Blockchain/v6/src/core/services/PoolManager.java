...
public class PoolManager implements UTXOPool, LeafPool, Serializable {
    ...
    public static void fillPool(long txNum) throws UnsupportedEncodingException {
        ...
        for(long i = 0; i < txNum; i++) {
            ...
            wallet = new Wallet();
            Transaction tx = new Transaction(fromHex(wallet.getPubAddress()), fromHex(new Wallet().getPubAddress()), fromHex(new BigInteger("10").toString(16)), null, wallet.getPrivKey());
            TxPool.put(tx);
        }
    }
