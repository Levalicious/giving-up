...
import org.levk.udp2p.util.FastByteComparisons;
...
import static org.levk.udp2p.serialization.TRENC.EMPTY_ELEMENT_TRENC;
...
public class TrieImpl implements Trie<byte[]> {
    ...
    public static final byte[] EMPTY_TRIE_HASH = blake2(TRENC.encode(EMPTY_BYTE_ARRAY));
    ...
    public final class Node {
        ...
        private ENCList parsedRlp = null;
        ...
        private Node(ENCList parsedRlp) {
        ...
        private byte[] encode(final int depth, boolean forceHash) {
            if (!dirty) {
                return hash != null ? TRENC.encode(hash) : rlp;
            } else {
                ...
                if (type == NodeType.BranchNode) {
                    if (depth == 1 && async) {
                        ...
                        encoded[16] = constantFuture(TRENC.encode(value));
                        try {
                        ...
                        for (int i = 0; i < 16; i++) {
                            ...
                            encoded[i] = child == null ? EMPTY_ELEMENT_TRENC : child.encode(depth + 1, false);
                        }
                        byte[] value = branchNodeGetValue();
                        encoded[16] = TRENC.encode(value);
                        ret = TRENC.encode(encoded);
                    }
                } else if (type == NodeType.KVNodeNode) {
                    ret = TRENC.encode(TRENC.encode(kvNodeGetKey().toPacked()), kvNodeGetChildNode().encode(depth + 1, false));
                } else {
                    byte[] value = kvNodeGetValue();
                    ret = TRENC.encode(TRENC.encode(kvNodeGetKey().toPacked()),
                            TRENC.encode(value == null ? EMPTY_BYTE_ARRAY : value));
                }
                if (hash != null) {
                    deleteHash(hash);
                }
        ...
        private void parse() {
            ...
            ENCList list = parsedRlp == null ? TRENC.decode(rlp) : parsedRlp;
